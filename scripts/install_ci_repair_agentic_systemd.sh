#!/usr/bin/env bash
# install_ci_repair_agentic_systemd.sh — render and optionally enable the
# every-other-day CI repair timer via clockwork.

set -euo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
PORTFOLIO_ROOT_DEFAULT="$(cd "${REPO_ROOT}/../.." && pwd)"
TEMPLATE_PATH="${REPO_ROOT}/config/clockwork/ci-repair-agentic.toml.template"
...
usage() {
  cat <<EOF
Usage: install_ci_repair_agentic_systemd.sh [options]

Render the every-other-day CI repair user-level systemd timer via clockwork and
...
systemctl --user enable --now ci-repair-agentic.timer
systemctl --user list-timers ci-repair-agentic.timer
