#!/usr/bin/env bash
# install_bug_sweep_agentic_systemd.sh — render and optionally enable the
# daily bug-sweep timer via clockwork.

set -euo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
PORTFOLIO_ROOT_DEFAULT="$(cd "${REPO_ROOT}/../.." && pwd)"
TEMPLATE_PATH="${REPO_ROOT}/config/clockwork/bug-sweep-agentic.toml.template"
...
PROVIDER_VALUE="${BUG_SWEEP_AGENTIC_PROVIDER:-auto}"
MODEL_VALUE="${BUG_SWEEP_AGENTIC_MODEL:-}"
RENDER_ONLY=0

usage() {
  cat <<EOF
Usage: install_bug_sweep_agentic_systemd.sh [options]

Render the daily bug-sweep user-level systemd timer via clockwork and enable it
by default.
...
systemctl --user enable --now bug-sweep-agentic.timer
systemctl --user list-timers bug-sweep-agentic.timer
