#!/usr/bin/env bash
# install_template_consolidation_agentic_systemd.sh — render and optionally
# enable the every-other-day template consolidation timer via clockwork.

set -euo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
PORTFOLIO_ROOT_DEFAULT="$(cd "${REPO_ROOT}/../.." && pwd)"
TEMPLATE_PATH="${REPO_ROOT}/config/clockwork/template-consolidation-agentic.toml.template"
...
PROVIDER_VALUE="${TEMPLATE_CONSOLIDATION_PROVIDER:-auto}"
MODEL_VALUE="${TEMPLATE_CONSOLIDATION_MODEL:-}"
RENDER_ONLY=0

usage() {
  cat <<EOF
Usage: install_template_consolidation_agentic_systemd.sh [options]

Render the every-other-day template consolidation user-level systemd timer via
clockwork and enable it by default.
...
systemctl --user enable --now template-consolidation-agentic.timer
systemctl --user list-timers template-consolidation-agentic.timer
