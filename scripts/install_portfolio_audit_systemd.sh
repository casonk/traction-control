#!/usr/bin/env bash
# install_portfolio_audit_systemd.sh — render and enable the daily portfolio
# governance audit timer via the shared clockwork scheduler utility.

set -euo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
PORTFOLIO_ROOT_DEFAULT="$(cd "${REPO_ROOT}/../.." && pwd)"
TEMPLATE_PATH="${REPO_ROOT}/config/clockwork/portfolio-audit-daily.toml.template"
...
usage() {
  cat <<EOF
Usage: install_portfolio_audit_systemd.sh [options]

Render the portfolio audit daily user-level systemd timer via clockwork
...
systemctl --user enable --now portfolio-audit-daily.timer
systemctl --user list-timers portfolio-audit-daily.timer
