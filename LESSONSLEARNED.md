...
- `portfolio-audit.sh` checked the AGENTS.md sudo boundary with a literal
  `grep -qF` for one repo's exact sentence. Three repos — `private-repository`,
  `private-repository`, and `private-repository` — each carried a correct, complete Sudo
...
- A governance audit that cries wolf is worse than no audit: the standing
  ...
  genuinely had no sudo boundary (`dot-github`, `private-repository`) sat in the
...
- Up-integrated from `private-repository`; related to the existing macOS shell
...
- Allowlisting ciphertext is safe only because the plaintext boundary is
  ...
  protects.
- Up-integrated from `private-repository`.
...
- Where a repo must diverge from the shared config, record why in the config
  itself. `private-repository` drops the `black` hook because black 26.x requires
