...
- [ ] [manual:2026-08-23] **Bring `install-harness` into the portfolio
  baseline** — It sits at `~/dev/install-harness`, outside `util-repos/` and
...
- [ ] [manual:2026-08-23] **Decide whether `.gitleaks.toml` without
  `secret-scan.yml` is acceptable** — `aterm-config` and `drm-sec` each ship a
