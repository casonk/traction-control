...
- [x] [manual:2026-06-17] **Tier 1** — Migrate 9 repos to reusable workflow callers (no blockers):
  tradility, private-repository, session-control, crew-chief, magneto, bit-byte-block → `python-ci.yml`;
...
- [x] [manual:2026-06-17] **Tier 5 — Per-repo decisions** (handle one at a time when ready):
  ...
  private-repository (add tachometer to [dev] extras, consolidate 2 jobs),
  ...
  private-repository (blocked on windshield being installable without GH_PRIVATE_REPO_PAT).
...
- [ ] [manual:2026-06-17] **private-repository private repo** — Create private GitHub repo for private-repository
  (`gh repo create private-repository --private`), set SSH remote, push pending commits.
...
- [ ] [manual:2026-06-11] Add post-refresh archive hooks for
  `private-repository` so existing `scripts/manage_storage_archives.py auto`
