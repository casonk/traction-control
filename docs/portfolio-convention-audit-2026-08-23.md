...
The portfolio is **25 repositories, not 24**. `private-repository` lives at
`~/dev/private-repository` rather than under `util-repos/` or `sec-repos/`, so
...
one repo's exact sentence. `private-repository`, `private-repository`, and `private-repository`
...
`private-repository` documents its boundary under `## Trust Boundary` and
...
| `stat -f` / `stat -c` fallbacks are not portability | `private-repository` |
| Secret scanners must be told ciphertext is not a leak | `private-repository` |
...
| Repo | State |
| --- | --- |
| `private-repository` | 18 gaps. No `AGENTS.md`, `SECURITY.md`, `LESSONSLEARNED.md`, `BACKLOG.md`, `LICENSE`, or `.pre-commit-config.yaml`. |
| `private-repository` | 17 gaps. Ships `.gitleaks.toml` and a baseline but no `secret-scan.yml` to run them — the config is dead. |
| `private-repository` | 16 gaps. No security tooling of any kind. |
| `private-repository` | 16 gaps. `BACKLOG.md` and `README.md` only; same dead-gitleaks-config problem as `private-repository`. |
...
| Repo | Gaps |
| --- | --- |
| `private-repository` | 12. `AGENTS.md` references no portfolio convention at all. Has CI (green as of 2026-08-23) and runs gitleaks via pre-commit, but has no `secret-scan.yml`, so full history is never scanned. |
| `dot-github` | 8. No sudo boundary; no `BACKLOG.md`; no issue/PR templates. |
| `private-repository` | 2. No `CHATHISTORY.md` boundary; no Local CI Verification. |
| `private-repository` | 2. Same two. |
| `private-repository` | 1. No Local CI Verification. |
